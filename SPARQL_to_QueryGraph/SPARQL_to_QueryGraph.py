...
def SPARQL_to_JSON(data, do_parsing):
    if do_parsing == True:
        ...
        for i in range(len(data)):
            ...
            f.write('0')
            f.close()
            os.system('node SPARQL.js/bin/SPARQL_to_JSON.js')
            ### 当互斥量为'1'时，说明写入完毕
            while True:
                f = open('mutex.txt', 'r')
                mutex = f.read()
...
def JSON_to_QueryGraph(data, do_parsing, do_debugging):
    if do_parsing == True:
        ...
        for i, data_temp in enumerate(data): 
            ...
            if do_debugging == True:
                ...
                new_data_temp.update({'graph_temp': copy.deepcopy(object_JSON_to_QueryGraph.graph_temp)})

            new_data_temp.update({'topic_entity': copy.deepcopy(topic_entity_temp)})
            new_data_temp.update({'core_inferential_chain': copy.deepcopy(core_inferential_chain_temp)})
            new_data_temp.update({'constraints': copy.deepcopy(constraints_temp)})
            new_data.append(copy.deepcopy(new_data_temp))
...
class class_JSON_to_QueryGraph:
    ...
    def get_topic_entity(self):
        if self.topic_entity == None:
            ...
            visited_vertex.append(copy.deepcopy(self.queue[-1]['value']))
            while len(self.queue) > 0:
                ...
                for edge_temp in graph_temp['and'][queue_temp['value']]['edge']:
                    ...
                    ### topic entity必须是NamedNode，且不能是type
                        ...
                        for edge_temp_temp in graph_temp['and'][edge_temp['endpoint']['value']]['edge']:
                            ...
                            self.topic_entity = self.append_mark(copy.deepcopy(edge_temp['endpoint']['value']))
                            ...
                            if edge_temp['direction'] == 'out':
                                ...
                                path_temp.append({
                                    'subject': self.append_mark(copy.deepcopy(queue_temp['value'])),
                                    'predicate': self.append_mark(copy.deepcopy(edge_temp['predicate']['value'])),
                                    'object': self.append_mark(copy.deepcopy(edge_temp['endpoint']['value']))
                                })
                                self.core_inferential_chain = path_temp
                            elif edge_temp['direction'] == 'in':
                                path_temp = copy.deepcopy(queue_temp['path'])
                                path_temp.append({
                                    'subject': self.append_mark(copy.deepcopy(edge_temp['endpoint']['value'])),
                                    'predicate': self.append_mark(copy.deepcopy(edge_temp['predicate']['value'])),
                                    'object': self.append_mark(copy.deepcopy(queue_temp['value']))
                                })
                                self.core_inferential_chain = path_temp
                            else: ### 其他未知情况
    ...
    def get_constraints(self):
        if self.constraints == None:
            ...
            ### 为了数据安全，做深复制
            graph_temp = copy.deepcopy(self.graph)
            self.graph_temp = graph_temp ### 用于调试
            self.constraints = []
            ...
            for i in range(1, len(core_inferential_chain_vertex_list)):
                for edge_temp in graph_temp['and'][core_inferential_chain_vertex_list[i]]['edge']:
                    ...
                    if edge_temp['type'] == 'triple':
                        constraints_temp.update({'type': copy.deepcopy(edge_temp['type'])})
                        constraints_temp.update({'Operator': 'Equal'})
                        ### 增加方向性，当前顶点做主语，'direction': 'out'；当前顶点做宾语，'direction': 'in'
                        constraints_temp.update({'Direction': copy.deepcopy(edge_temp['direction'])})
                    elif edge_temp['type'] == 'operation':
                        constraints_temp.update({'type': copy.deepcopy(edge_temp['type'])})
                        constraints_temp.update({'Operator': copy.deepcopy(operator_to_operator[edge_temp['operator']])})
                    ...
                    constraints_temp.update({'Argument': self.append_mark(copy.deepcopy(edge_temp['endpoint']['value']))})
                    ...
                    if edge_temp['predicate'] != None:
                        constraints_temp.update({'NodePredicate': self.append_mark(copy.deepcopy(edge_temp['predicate']['value']))})
                    else:
                        constraints_temp.update({'NodePredicate': edge_temp['predicate']})
            ...
            if graph_temp['order_limit_offset'] != None:
                ...
                if 'termType' in graph_temp['order_limit_offset']['order'][0]['expression']:
                    ...
                    if graph_temp['order_limit_offset']['order'][0]['expression']['value'] == 'date':
                        order_limit_offset.update({'ValueType': 'DateTime'})
                    else:
                        order_limit_offset.update({'ValueType': 'String'})
                    ...
                    if vertex == core_inferential_chain_vertex_list_temp:
                        ...
                        for edge_temp in graph_temp['and'][vertex]['edge']:
                            if edge_temp['endpoint']['value'] == core_inferential_chain_vertex_list_temp:
                                order_limit_offset.update({'SourceNodeIndex': copy.deepcopy(i)})
                                order_limit_offset.update({'NodePredicate': self.append_mark(copy.deepcopy(edge_temp['predicate']['value']))})
                        ### 跳出双循环的方法！
                                break
                        else:
                            continue
    ...
    def append_mark(self, vertex):
        ...
        if vertex != None:
            ...
            if (vertex in graph_temp['and']) and (graph_temp['and'][vertex]['termType'] == 'Variable') and (vertex[0] != '?'):
                return '?' + vertex
            ### 实体或类型URI
            ### 谓语的URI不满足 vertex in graph_temp['and']，而entity一定满足 (len(vertex) >= 7) and (vertex[:7] == 'http://')
            elif (len(vertex) >= 7) and (vertex[:7] == 'http://') and ((vertex[0] != '<') or (vertex[-1] != '>')):
                return '<' + vertex + '>'
    ...
    def remove_mark(self, vertex):
        ...
        if vertex != None:
            ...
            ### 实体或类型URI
            ### 谓语的URI不满足 vertex in graph_temp['and']，而entity一定满足 (len(vertex) > 7) and (vertex[:7] == 'http://')
            elif ((vertex[0] == '<') and (vertex[-1] == '>')) and (len(vertex[1:-1]) >= 7) and (vertex[1:8] == 'http://'):
                return vertex[1:-1]
...
if __name__=="__main__":
    ...
    data = JSON_to_QueryGraph(data, do_JSON_to_QueryGraph, do_debugging)