def read_QueryGraph(QueryGraph_data_path):
    f = open(QueryGraph_data_path, 'r')
...
def read_upstream_data(process_upstream_data, upstream_entity_data_path, upstream_core_inferential_chain_data_path, upstream_golden_core_inferential_chain_data_path):
    if process_upstream_data == True:
        ...
        if upstream_golden_core_inferential_chain_data_path != None:
            ...
            for item_temp in f_reader_temp['items']:
                ...
                if len(item_temp['chain']) == 3:
                    if item_temp['chain'][0] == 'VAR':
                        ...
                        core_inferential_chain_direction = ['in']
                    else: ### item_temp['chain'][2] == 'VAR'
                        topic_entity_temp = copy.deepcopy(item_temp['chain'][0])
                        core_inferential_chain_direction = ['out']
                    core_inferential_chain_temp = [copy.deepcopy(item_temp['chain'][1])]
                    candidate_temp = {
                        'topic_entity': copy.deepcopy(topic_entity_temp), ### 'topic_entity'是str
                        'core_inferential_chain': copy.deepcopy(core_inferential_chain_temp), ### 'core_inferential_chain'是list，第i个元素是第i个variable与前一个位置之间的predicate
                        'core_inferential_chain_direction': copy.deepcopy(core_inferential_chain_direction), 
                        ### 'core_inferential_chain_direction'是list，第i个元素是第i个variable与前一个位置之间的边的方向
                        'variable_value': [],
                ...
                ### 2跳
                    ...
                    if item_temp['chain'][0] == 'VAR1':
                        ...
                        topic_entity_temp = copy.deepcopy(item_temp['chain'][0])
                        core_inferential_chain_direction = ['out']
                    if item_temp['chain'][3] == 'VAR1':
                        core_inferential_chain_direction.append('out')
                    else: ### item_temp['chain'][5] == 'VAR1'
                        core_inferential_chain_direction.append('in')
                    core_inferential_chain_temp = [copy.deepcopy(item_temp['chain'][1]), copy.deepcopy(item_temp['chain'][4])]
                    candidate_temp = {
...
'''
每一个question（同一个'id'值）->
每一套topic entity、core inferential chain ->
每一套variable取值 ->
该question的每一个mention ->
该mention的每一个entity ->
该套variable取值的每一个位置的variable ->
该entity的每一个predicate ->
对应0~1个constraints
（1）一个entity的所有constraints之间需要组合，共2^N个组合
（2）一个question的多个mention的constraints集合之间需要组合求并集，共M^N个组合
（同一个mention下，constraints集合之间不组合。
因为同一个entity的constraints在（1）中已经组合，不同entity的constraints不能组合）
下面的代码是必须的
### 给每个mention加一个空集，目的是使得单个constraints集合可以保留、不相邻的mention之间也能实现组合
constraints_set_for_each_mention[mention_temp].append(set())
（1）（2）中的组合，再加上最后的constraints集合去重，
可以实现各个variable value的constraints集合的组合求交集，也就是同一个constraints集合可以搜索到多个variable value
'''
import copy
import itertools
import time
...
import math
import numpy as np
### max_constraints_number用于限制每个constraints集合内constraints元素的最大数量
### max_computation_time用于限制每个core inferential chain可接受的最大计算时间，单位为秒。如果超过这个最大计算事件，则跳过该core inferential chain
def constraints_candidate_generation(make_candidate, QueryGraph, DBpedia_graph, entity_data, core_inferential_chain_data, max_constraints_number=2, max_computation_time=10, constraints_type=['triple']):
    if make_candidate == True:
        ...
        for i, id_temp in enumerate(core_inferential_chain_data.get_id_list()):
            ...
            for core_inferential_chain_temp in core_inferential_chain_data.get_core_inferential_chain_list(id_temp):
                ...
                for p1, variable_value_temp in enumerate(core_inferential_chain_temp['variable_value']):
                    ...
                    for mention_temp, entity_to_string_temp in entity_data.get_entity_to_string_dict(id_temp).items():
                        ...
                        for entity_temp, string_temp in entity_to_string_temp.items():
                            ...
                            constraints_set_for_each_entity = []
                            ...
                            for j, variable_value_temp_temp in enumerate(variable_value_temp):
                                ...
                                if variable_value_temp_temp in DBpedia_graph.get_endpoint_set(entity_temp):
                                    ...
                                    ### 一个constraints candidate里，在一个variable位置上，一对entity、endpoint之间可能有多种predicate。因此这里是多个edge组成的列表edge_list
                                    for edge_list_temp in edge_list:
                                        ### 如果当前constraints就是第1个variable与topic entity相连的那条边，就跳过该constraints
                                        if (j == 0) and \
                                            (entity_temp == core_inferential_chain_temp['topic_entity']) and \
                                            (edge_list_temp['predicate'] == core_inferential_chain_temp['core_inferential_chain'][0]) and \
                                            (DBpedia_graph.get_direction(entity_temp, variable_value_temp_temp, edge_list_temp['predicate']) == core_inferential_chain_temp['core_inferential_chain_direction'][0]):
                                            mention_is_topic_entity = True
                                            continue
                                        constraints_set_for_each_entity.append({str([[
                                            'triple', ### 'type':  
                                            'Equal', ### 'Operator': 
                                        ...
                                        '''
                            ### 该套variable value下，所有constraints可以单独存在；相同位置variable的constraints之间、不同位置variable的constraints之间都可以进行组合
                            constraints_set_for_each_entity_combination_union = []
                            for k in range(1, min(len(constraints_set_for_each_entity), max_constraints_number) + 1):
                                for constraints_set_for_each_entity_combination_temp in itertools.combinations(constraints_set_for_each_entity, k):
                                    constraints_set_for_each_entity_combination_temp_union = set().union(*constraints_set_for_each_entity_combination_temp)
                                    if (len(constraints_set_for_each_entity_combination_temp_union) <= max_constraints_number) and \
                                        (len(constraints_set_for_each_entity_combination_temp_union) > 0) and \
                                        constraints_set_for_each_entity_combination_temp_union not in constraints_set_for_each_entity_combination_union:
                                        constraints_set_for_each_entity_combination_union.append(copy.deepcopy(constraints_set_for_each_entity_combination_temp_union))

                            ### 更新的同时实现限制constraints数量、去除空集、去重
                            for constraints_set_for_each_entity_combination_union_temp in constraints_set_for_each_entity_combination_union:
                                if (len(constraints_set_for_each_entity_combination_union_temp) <= max_constraints_number) and \
                                    (len(constraints_set_for_each_entity_combination_union_temp) > 0) and \
                                    (constraints_set_for_each_entity_combination_union_temp not in constraints_set_for_each_mention[mention_temp]):
                                    constraints_set_for_each_mention[mention_temp].append(copy.deepcopy(constraints_set_for_each_entity_combination_union_temp))

                            '''
                            ### 另一种写法
                                    constraints_set_list_combination_union = []
                                    for k in range(1, min(len(constraints_set_list), max_constraints_number) + 1):
                                        for constraints_set_list_combination_temp in itertools.combinations(constraints_set_list, k):
                                            constraints_set_list_combination_temp_union = set().union(*constraints_set_list_combination_temp)
                                            if (len(constraints_set_list_combination_temp_union) <= max_constraints_number) and \
                                                (len(constraints_set_list_combination_temp_union) > 0) and \
                                                constraints_set_list_combination_temp_union not in constraints_set_list_combination_union:
                                                constraints_set_list_combination_union.append(constraints_set_list_combination_temp_union)
                                    ...
                                    for constraints_set_for_each_entity_temp_temp in constraints_set_for_each_entity:
                                        ...
                                        for constraints_set_list_combination_union_temp in constraints_set_list_combination_union:
                                            constraints_set_for_each_entity_temp_temp_copy = copy.deepcopy(constraints_set_for_each_entity_temp_temp)
                                            constraints_set_for_each_entity_temp_temp_copy.update(constraints_set_list_combination_union_temp)
                                            ### 更新的同时实现限制constraints数量、去除空集、去重
                                            if (len(constraints_set_for_each_entity_temp_temp_copy) <= max_constraints_number) and \
                                                (len(constraints_set_for_each_entity_temp_temp_copy) > 0) and \
                                                constraints_set_for_each_entity_temp_temp_copy not in constraints_set_for_each_entity_temp:
                                                constraints_set_for_each_entity_temp.append(copy.deepcopy(constraints_set_for_each_entity_temp_temp_copy))
                                    ### 如果constraints_set_list非空
                                    if len(constraints_set_for_each_entity_temp) > 0:
                                        constraints_set_for_each_entity = constraints_set_for_each_entity_temp
                                        ### 给每个variable位置加1个空集，这样大于1跳的core inferential chain，不同variable位置的constraints可以单独存在，也可以进行组合
                                        constraints_set_for_each_entity.append(set())

                            ### 更新的同时实现限制constraints数量、去除空集、去重
                            for constraints_set_for_each_entity_temp in constraints_set_for_each_entity:
                                if (len(constraints_set_for_each_entity_temp) <= max_constraints_number) and \
                                    (len(constraints_set_for_each_entity_temp) > 0) and \
                        ...
                        ### 给每个mention加一个空集，这样不相邻的mention之间也能实现组合
                        constraints_set_for_each_mention[mention_temp].append(set())
                    '''
                    ...
                    for mention_temp, constraints_set_for_each_mention_temp in constraints_set_for_each_mention.items():
                        ...
                        for constraints_set_mention_combination_temp_temp in constraints_set_mention_combination:
                            ...
                            for constraints_set_for_each_mention_temp_temp in constraints_set_for_each_mention_temp:
                                ...
                                ### 更新的同时实现限制constraints数量、去重,不去除空集
                                if (len(constraints_set_mention_combination_temp_temp_copy) <= max_constraints_number) and \
                                    (constraints_set_mention_combination_temp_temp_copy not in constraints_set_mention_combination_temp):
                                    constraints_set_mention_combination_temp.append(copy.deepcopy(constraints_set_mention_combination_temp_temp_copy))
                        ### 如果constraints_set_for_each_mention_temp非空
                    ...
                    for constraints_set_mention_combination_temp in constraints_set_mention_combination:
                        if (len(constraints_set_mention_combination_temp) <= max_constraints_number) and \
                            constraints_set_mention_combination_temp not in constraints_set:
                            constraints_set.append(copy.deepcopy(constraints_set_mention_combination_temp))

                    '''
                    ### 另一种写法
                    mention_list = list(constraints_set_for_each_mention.keys())
                    for j in range(0, min(len(mention_list), max_constraints_number) + 1):
                        for mention_list_combination_temp in itertools.combinations(mention_list, j):
                            constraints_set_mention_combination = [set()]
                            ### 遍历每种mention
                            for mention_temp in mention_list_combination_temp:
                                constraints_set_mention_combination_temp = []
                                ### 遍历当前积累的所有constraints set
                                for constraints_set_mention_combination_temp_temp in constraints_set_mention_combination:
                                    ### 遍历每种mention的所有constraints set（每个constraints set对应1个entity）
                                    for constraints_set_for_each_mention_temp in constraints_set_for_each_mention[mention_temp]:
                                        constraints_set_mention_combination_temp_temp_copy = copy.deepcopy(constraints_set_mention_combination_temp_temp)
                                        ### 求并集
                                        constraints_set_mention_combination_temp_temp_copy.update(copy.deepcopy(constraints_set_for_each_mention_temp))
                                        ### 更新的同时实现限制constraints数量、去除空集、去重
                                        if (len(constraints_set_mention_combination_temp_temp_copy) <= max_constraints_number) and \
                                            (len(constraints_set_mention_combination_temp_temp_copy) > 0) and \
                                            (constraints_set_mention_combination_temp_temp_copy not in constraints_set_mention_combination_temp):
                                            constraints_set_mention_combination_temp.append(copy.deepcopy(constraints_set_mention_combination_temp_temp_copy))
                                ### 如果constraints_set_for_each_mention_temp非空
                                if len(constraints_set_mention_combination_temp) > 0:
                ...
                '''
                ### 每个constraints set对应一种constraints candidate，从而生成一种Query Graph candidate
                progress_bar3 = progressbar.ProgressBar(max_value=len(constraints_set))
                for p3, constraints_set_temp in enumerate(constraints_set):
                    constraints_temp = []
                    while len(constraints_set_temp) > 0:
                        constraints_set_temp_list = copy.deepcopy(eval(constraints_set_temp.pop()))
                        constraints_set_temp_dict = {}
                        for j in range(len(constraints_set_temp_list[0])):
                            constraints_set_temp_dict.update({
                                copy.deepcopy(constraints_set_temp_list[1][j]): copy.deepcopy(constraints_set_temp_list[0][j])
                            })
                        constraints_temp.append(copy.deepcopy(constraints_set_temp_dict))
                    query_graph_candidate_temp = {
                        ...
                        'core_inferential_chain_direction': copy.deepcopy(core_inferential_chain_temp['core_inferential_chain_direction']),
        ...
        QueryGraph = json.load(f) ### QueryGraph为list->dict类型
        f.close()
    ...
    if make_label == True:
        ...
        for QueryGraph_temp in QueryGraph:
            ...
            for query_graph_candidate_temp in QueryGraph_temp['query_graph_candidate']:
                ...
                for core_inferential_chain_temp in query_graph_candidate_temp['core_inferential_chain']:
...
def statistical_result(QueryGraph):
    ...
    positive_sample_number_per_question = []
    constraints_candidate_number_per_query_graph_candidate = []
    question_number_for_each_positive_sample_number = {}

    query_graph_candidate_number_per_question_for_each_hop_count = {}
    positive_sample_ratio_per_question_for_each_hop_count = {} 
    positive_sample_number_per_question_for_each_hop_count = {}
    constraints_candidate_number_per_query_graph_candidate_for_each_hop_count = {}
    for QueryGraph_temp in QueryGraph:
        if QueryGraph_temp['available_for_model'] == True:
            available_question_number += 1
            
            query_graph_candidate_number_per_question_temp = 0
            positive_sample_ratio_per_question_temp = 0
            positive_sample_number_per_question_temp = 0
            
            query_graph_candidate_number_per_question_for_each_hop_count_temp = {}
            positive_sample_ratio_per_question_for_each_hop_count_temp = {} 
            positive_sample_number_per_question_for_each_hop_count_temp = {}
            for query_graph_candidate_temp in QueryGraph_temp['query_graph_candidate']:
                query_graph_candidate_number_per_question_temp += 1
                positive_sample_ratio_per_question_temp += query_graph_candidate_temp['label'] / len(QueryGraph_temp['query_graph_candidate'])
                positive_sample_number_per_question_temp += query_graph_candidate_temp['label']
                constraints_candidate_number_per_query_graph_candidate.append(len(query_graph_candidate_temp['constraints']))
                
                hop_count = len(query_graph_candidate_temp['core_inferential_chain'])
                if hop_count not in query_graph_candidate_number_per_question_for_each_hop_count_temp:
                    query_graph_candidate_number_per_question_for_each_hop_count_temp.update({hop_count: 0})
                query_graph_candidate_number_per_question_for_each_hop_count_temp[hop_count] += 1
                if hop_count not in positive_sample_ratio_per_question_for_each_hop_count_temp:
                    positive_sample_ratio_per_question_for_each_hop_count_temp.update({hop_count: []})
                positive_sample_ratio_per_question_for_each_hop_count_temp[hop_count].append(query_graph_candidate_temp['label'])
                if hop_count not in positive_sample_number_per_question_for_each_hop_count_temp:
                    positive_sample_number_per_question_for_each_hop_count_temp.update({hop_count: 0})
                positive_sample_number_per_question_for_each_hop_count_temp[hop_count] += query_graph_candidate_temp['label']
                if hop_count not in constraints_candidate_number_per_query_graph_candidate_for_each_hop_count:
                    constraints_candidate_number_per_query_graph_candidate_for_each_hop_count.update({hop_count: []})
                constraints_candidate_number_per_query_graph_candidate_for_each_hop_count[hop_count].append(len(query_graph_candidate_temp['constraints']))

            query_graph_candidate_number_per_question.append(query_graph_candidate_number_per_question_temp)
            positive_sample_ratio_per_question.append(positive_sample_ratio_per_question_temp)
            positive_sample_number_per_question.append(positive_sample_number_per_question_temp)

            if positive_sample_number_per_question_temp not in question_number_for_each_positive_sample_number:
                question_number_for_each_positive_sample_number.update({positive_sample_number_per_question_temp: 0})
            question_number_for_each_positive_sample_number[positive_sample_number_per_question_temp] += 1
            
            for keys_temp, values_temp in query_graph_candidate_number_per_question_for_each_hop_count_temp.items():
                if keys_temp not in query_graph_candidate_number_per_question_for_each_hop_count.keys():
                    query_graph_candidate_number_per_question_for_each_hop_count.update({keys_temp: []})
                query_graph_candidate_number_per_question_for_each_hop_count[keys_temp].append(values_temp)
            for keys_temp, values_temp in positive_sample_ratio_per_question_for_each_hop_count_temp.items():
                if keys_temp not in positive_sample_ratio_per_question_for_each_hop_count.keys():
                    positive_sample_ratio_per_question_for_each_hop_count.update({keys_temp: []})
                ### values_temp 一定非空
                positive_sample_ratio_per_question_for_each_hop_count[keys_temp].append(np.mean(values_temp))
            for keys_temp, values_temp in positive_sample_number_per_question_for_each_hop_count_temp.items():
                if keys_temp not in positive_sample_number_per_question_for_each_hop_count.keys():
                    positive_sample_number_per_question_for_each_hop_count.update({keys_temp: []})
                positive_sample_number_per_question_for_each_hop_count[keys_temp].append(values_temp)
            

    print('available_question_number: %d' % (available_question_number))
    print('content mean std min max')
    ...
    print('positive_sample_number_per_question: %.2f %.2f %.2f %.2f' % (
        np.mean(positive_sample_number_per_question),
        np.std(positive_sample_number_per_question),
        min(positive_sample_number_per_question),
        max(positive_sample_number_per_question)
    ))
    ...
    for hop_count in query_graph_candidate_number_per_question_for_each_hop_count.keys():
        print('The number of hops: %d' % (hop_count))
        print('query_graph_candidate_number_per_question: %.2f %.2f %.2f %.2f' % (
            np.mean(query_graph_candidate_number_per_question_for_each_hop_count[hop_count]),
            np.std(query_graph_candidate_number_per_question_for_each_hop_count[hop_count]),
            min(query_graph_candidate_number_per_question_for_each_hop_count[hop_count]),
            max(query_graph_candidate_number_per_question_for_each_hop_count[hop_count])
        ))
        print('positive_sample_ratio_per_question: %.4f %.4f %.4f %.4f' % (
            np.mean(positive_sample_ratio_per_question_for_each_hop_count[hop_count]),
            np.std(positive_sample_ratio_per_question_for_each_hop_count[hop_count]),
            min(positive_sample_ratio_per_question_for_each_hop_count[hop_count]),
            max(positive_sample_ratio_per_question_for_each_hop_count[hop_count])
        ))
        print('positive_sample_number_per_question: %.2f %.2f %.2f %.2f' % (
            np.mean(positive_sample_number_per_question_for_each_hop_count[hop_count]),
            np.std(positive_sample_number_per_question_for_each_hop_count[hop_count]),
            min(positive_sample_number_per_question_for_each_hop_count[hop_count]),
            max(positive_sample_number_per_question_for_each_hop_count[hop_count])
        ))
        print('constraints_candidate_number_per_query_graph_candidate: %.2f %.2f %.2f %.2f' % (
            np.mean(constraints_candidate_number_per_query_graph_candidate_for_each_hop_count[hop_count]),
            np.std(constraints_candidate_number_per_query_graph_candidate_for_each_hop_count[hop_count]),
            min(constraints_candidate_number_per_query_graph_candidate_for_each_hop_count[hop_count]),
            max(constraints_candidate_number_per_query_graph_candidate_for_each_hop_count[hop_count])
        ))
    print('positive sample number, question number')
    for positive_sample_number_temp, question_number_temp in question_number_for_each_positive_sample_number.items():
        print(positive_sample_number_temp, question_number_temp)

if __name__=="__main__":
    QueryGraph_data_path = '../SPARQL_to_QueryGraph/train_merged_QALD_in_QueryGraph.json'
    make_graph = False
    process_upstream_data = True
    ...
    QueryGraph = read_QueryGraph(QueryGraph_data_path)
