...
class class_DBpedia_graph:
    ...
    ### 返回类型为 dict
    def endpoint_to_edge(self, entity, endpoint):
        ### 检验entity是否在DBpedia_graph中
        if self.is_existing(entity) == False:
            return None
            ...
            if entity not in self.endpoint_index_cache_key_set:
                ...
                for i, edge_temp in enumerate(self.get_edge(entity)):
                    self.endpoint_index_cache[entity].update({
                        edge_temp['endpoint']: i
                    })
                self.endpoint_index_cache_key_set.add(entity)
            return self.get_edge(entity)[self.endpoint_index_cache[entity][endpoint]]
        else:
            return None

    def get_direction(self, entity, endpoint):
        ### 检验entity是否在DBpedia_graph中
        if self.is_existing(entity) == False:
            return None
        ### 检验endpoint是否与entity相连
        if endpoint in self.get_endpoint_set(entity):
            return self.endpoint_to_edge(entity, endpoint)['direction']
        else:
            return None

    def get_reverse_direction(self, entity, endpoint):
        ### 检验entity是否在DBpedia_graph中
        if self.is_existing(entity) == False:
            return None
        ### 检验endpoint是否与entity相连
        if endpoint in self.get_endpoint_set(entity):
            return 'out' if self.endpoint_to_edge(entity, endpoint)['direction'] == 'in' else 'in'
        else:
            return None
...
import math
'''
max_computation用于估算每个core inferential chain可接受的最大计算量。如果超过这个最大计算量，则跳过该core inferential chain
...
因此，默认值max_computation=10**7一般对应1分钟
'''
def constraints_candidate_generation(make_candidate, QueryGraph, DBpedia_graph, entity_data, core_inferential_chain_data, max_computation=10**7, constraints_type=['triple']):
    if make_candidate == True:
        ...
        for i, id_temp in enumerate(core_inferential_chain_data.get_id_list()):
            ...
            for core_inferential_chain_temp in core_inferential_chain_data.get_core_inferential_chain_list(id_temp):
                ...
                if variable_value_number > max_computation:
                    print('The amount of computation is more than max_computation')
                    print('core inferential chain: ', core_inferential_chain_temp['core_inferential_chain'])
                    continue
                start_time = time.time()
                print_core_inferential_chain = False
                ...
                for p1, variable_value_temp in enumerate(core_inferential_chain_temp['variable_value']):
                        ...
                        for entity_temp, string_temp in entity_to_string_temp.items():
                            ...
                            for j, variable_value_temp_temp in enumerate(variable_value_temp):
                                ...
                                if not ((variable_value_temp_temp[0] == '<') and (variable_value_temp_temp[-1] == '>')):
                                    continue
                                ### DBpedia_graph.get_endpoint_set() 和 DBpedia_graph.endpoint_to_edge() 的 entity参数 应该传入entity_data的entity，而不传入core_inferential_chain_data的variable的取值entity
                                ...
                                if variable_value_temp_temp in DBpedia_graph.get_endpoint_set(entity_temp):
                                    ...
                                    edge_temp = DBpedia_graph.endpoint_to_edge(entity_temp, variable_value_temp_temp)
                                    constraints_set_for_each_entity.add(str([[
                                        'triple', ### 'type':  
                                        'Equal', ### 'Operator': 
                                        DBpedia_graph.get_reverse_direction(entity_temp, variable_value_temp_temp), ### 'Direction': 对'Direction'进行翻转，实际上是 'out' if edge_temp['direction'] == 'in' else 'in',
                                        'Entity', ### 'ArgumentType': 
                                        copy.deepcopy(entity_temp), ### 'Argument': 
                                        copy.deepcopy(string_temp), ### 'EntityName': 
                                        j, ### 'SourceNodeIndex': 
                                        copy.deepcopy(edge_temp['predicate']), ### 'NodePredicate': 
                                        None ### 'ValueType': 
                                    ...
                                    print([
                                        'triple', ### 'type':  
                                        'Equal', ### 'Operator': 
                                        'out' if edge_temp['direction'] == 'in' else 'in', ### 'Direction': ，对'Direction'进行翻转
                                        'Entity', ### 'ArgumentType': 
                                        copy.deepcopy(entity_temp), ### 'Argument': 
                                        copy.deepcopy(string_temp), ### 'EntityName': 
                                        j, ### 'SourceNodeIndex': 
                                    ...
                                    '''
                            ### 更新的同时实现去除空集和去重
                            if (len(constraints_set_for_each_entity) > 0) and \
                                (constraints_set_for_each_entity not in constraints_set_for_each_mention[mention_temp]):
                                constraints_set_for_each_mention[mention_temp].append(copy.deepcopy(constraints_set_for_each_entity))
                    '''
                    ...
                    if (time.time() - start_time > 5*60) and (print_core_inferential_chain == False):
                        print('core inferential chain: ', core_inferential_chain_temp['core_inferential_chain'])
                        print_core_inferential_chain = True
                ...
                if variable_value_number * constraints_set_average_number_per_variable_value >= max_computation:
                    print('The amount of computation is more than max_computation')
                    print('core inferential chain: ', core_inferential_chain_temp['core_inferential_chain'])
                    continue
                ...
                ### 对所有constraints集合组合求交集，结果是交集（集合）组成的列表
                ...
                # max_subset = 2
                ### 根据 max_computation、variable_value_number、constraints_set_average_number_per_variable_value，动态计算 max_subset
                ### 该core inferential chain估计的最大计算量为 (variable_value_number ^ max_subset) * (constraints_set_average_number_per_variable_value ^ max_subset)
                eps = 10**(-8)
                max_subset = int(math.log(max_computation + eps, variable_value_number * constraints_set_average_number_per_variable_value + eps))
                for j in range(0, min(len(constraints_set) + 1, max_subset + 1)):
                    ...
                    for constraints_set_combinations_intersection_temp_temp in constraints_set_combinations_intersection_temp:
                        if constraints_set_combinations_intersection_temp_temp not in constraints_set_combinations_intersection:
                            constraints_set_combinations_intersection.append(constraints_set_combinations_intersection_temp_temp)
                '''
                ### 对交集去重
...
def statistical_result(QueryGraph):
    ...
    print('positive_sample_ratio_per_question: %.2f %.2f %.2f %.2f' % (
